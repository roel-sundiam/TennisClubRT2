...
/**
 * Migration Script: Court Usage Report Data
 * 
 * This script migrates the static court usage data from the hardcoded array
 * to the new CourtUsageReport MongoDB collection.
 * 
 * Usage:
 * node scripts/migrateCourtUsageData.js [--force]
 * 
 * Options:
 * --force: Clear existing data before migration
 */
...
// Static data from staticReportController.ts
const staticData = [
...
// Define the schema for the migration
const courtUsageSchema = new mongoose.Schema({
...
async function migrateData(force = false) {
  try {
    console.log('🚀 Starting Court Usage Report data migration...\n');
    ...
    if (existingCount > 0 && !force) {
      ...
      console.log('   Example: node scripts/migrateCourtUsageData.js --force\n');
      return;
    }
    ...
    console.log(`📊 Migrating ${staticData.length} member records...`);
    
    const transformedData = staticData.map(member => ({
    ...
    for (let i = 0; i < transformedData.length; i += batchSize) {
      ...
      try {
        await CourtUsageReport.insertMany(batch, { ordered: false });
        insertedCount += batch.length;
        
        console.log(`   ✅ Inserted batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(transformedData.length/batchSize)} (${insertedCount}/${transformedData.length} records)`);
        
      } catch (error) {
        // Handle potential duplicate key errors gracefully
        if (error.code === 11000) {
          console.log(`   ⚠️  Some records in batch ${Math.floor(i/batchSize) + 1} already exist, skipping duplicates`);
        } else {
          throw error;
        }
      }
